...
def response(text, BOT, debugMode, reset, recent, name):
    if (debugMode == "trace" and reset == 'reset' and recent == 'recent'):
        ...
        fullresponse = API.talk(USER_KEY, APP_ID, HOST, BOT, text, clientID=name)
...
def talk(BOT, debugMode, recent, reset, name):
    ...
    while not done:
        if question == "quit":
            ...
            response(question, BOT, debugMode, reset, recent, name)
...
def main():
    if len(sys.argv) < 3:
        print 'usage: ./Talk.py BOT debugMode recent reset clientname'
        sys.exit(1)
    ...
    reset = sys.argv[4]
    name = sys.argv[5]
    talk(BOT, debugMode, recent, reset, name)
